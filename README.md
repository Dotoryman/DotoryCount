...
## 0.4.0 아이폰 영상 시안 (정식 릴리즈 전)

- 앱 아이콘의 유리병과 도토리 표현을 기준으로 만든 낙하 영상이 홈 화면의 중심입니다.
- 화면 탭, 앱 열기, 다른 화면에서 돌아오기에 영상을 다시 재생합니다.
- 날짜와 기념일 계산 및 로컬 저장 데이터는 기존대로 유지합니다.
- 현재 병 영상은 하나의 고정된 시안입니다. 실제 날짜별 채움 단계와 황금도토리의
  영상 표현은 아직 구현되지 않았으므로 정식 0.4.0 릴리즈로 간주하지 않습니다.
